...
export async function GET(req) {
  try {
    console.log("in backend of opportunities");
    ...
    const [org1Res, org2Res] = await Promise.all([
      axios.get(`${org1Instance}/services/apexrest/opportunities`, {
        headers: { Authorization: `Bearer ${org1Token}` },
      }),
      axios.get(`${org2Instance}/services/apexrest/opportunities`, {
        headers: { Authorization: `Bearer ${org2Token}` },
      }),
    ]);

    const org1Data = org1Res.data.map((o) => ({ ...o, org: "Org 1" }));
    const org2Data = org2Res.data.map((o) => ({ ...o, org: "Org 2" }));
